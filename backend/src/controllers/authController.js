...
async function register(req, res, next) {
  try {
    ...
    const existing = await pool.query('SELECT user_id, is_verified FROM users WHERE email = $1', [email]);
    if (existing.rows.length) {
      ...
      return success(res, { email, user_id: existing.rows[0].user_id },
        'Account pending verification. A new OTP has been sent.', 200);
    }
    ...
    try {
      ...
      const { rows: [user] } = await client.query(
        ...
        [name, email, hashed, phone || null, language_preference || 'en', otp, otpExpiry]
      );

      const { rows: [role] } = await client.query(`SELECT role_id FROM roles WHERE role_name = 'patient'`);
      await client.query(`INSERT INTO user_roles (user_id, role_id) VALUES ($1,$2)`, [user.user_id, role.role_id]);
      await client.query(`INSERT INTO patient_profiles (patient_id) VALUES ($1)`, [user.user_id]);

      await client.query('COMMIT');
      sendOtpEmail(email, name, otp).catch(e => console.error('[EMAIL]', e.message));

      return success(res, { email, user_id: user.user_id },
        'Registration successful. Check your email for a verification code.', 201);
    } catch (e) {
...
async function verifyOtp(req, res, next) {
  try {
    ...
    if (user.is_verified) return error(res, 'Account already verified. Please log in.', 400);
    if (user.otp_code !== String(otp)) return error(res, 'Invalid verification code.', 400);
    ...
    return success(res,
      { user: { user_id: user.user_id, name: user.name, email: user.email, roles: roleRows.map(r => r.role_name) } },
...
async function resendOtp(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(`SELECT user_id, name, is_verified FROM users WHERE email = $1`, [email]);
    const user = rows[0];
    if (!user)            return error(res, 'Account not found.', 404);
    ...
    sendOtpEmail(email, user.name, otp).catch(e => console.error('[EMAIL]', e.message));
...
async function login(req, res, next) {
  try {
    ...
    if (!user.is_verified) {
      ...
      return error(res, 'Email not verified. A new code has been sent to your email.', 403);
    }
    ...
    return success(res, { user: { ...safeUser, roles: roleRows.map(r => r.role_name) } }, 'Login successful.');
  } catch (err) { next(err); }
}
